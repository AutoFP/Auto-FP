...
def tpot_ps_job(classifier, pretrain_mins, round_th, seed, dataset):
    simple_config_dict = {

        # Preprocesssors
        'sklearn.preprocessing.Binarizer': {
            'threshold': np.arange(0.0, 1.01, 0.2)
        },
    ...
    # max_eval_time_mins, max_times_mins, populaton_size = (1, 2, 10), (1, 10, 20), (6, 60, 100)
    tpot = TPOTClassifier(config_dict=simple_config_dict, cv=ps, generations=None, max_eval_time_mins=1, max_time_mins=int(pretrain_mins),
            n_jobs=1, population_size=20, random_state=int(seed), verbosity=1)
    tpot.fit(X_train_valid, y_train_valid)
    print(tpot.score(X_test, y_test))
    tpot.export('tpot_' + classifier + '_ps_' + pretrain_mins + 'min/round_' + round_th + '/seed_' + seed + '/tpot_' + dataset + '_pipeline.py')
...
if __name__ == "__main__":
    ...
    result = Parallel(n_jobs=75)(delayed(tpot_ps_job)(classifier, pretrain_mins, round_th, seed, dataset) \
