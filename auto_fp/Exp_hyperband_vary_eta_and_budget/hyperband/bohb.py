...
from utils import set_env, load_data, get_pipe, make_output_dir_hb_vary_eta_and_budget
...
class MyWorker(Worker):
    ...
    def compute(self, config, budget, **kwargs):
        ...
        if (budget == self.max_budget):
            f = open(f'{self.output_dir}/bohb_wallock_{self.seed}.csv', 'a')
            f.write(str(global_mid - self.global_start) + "\n")
            f = open(f'{self.output_dir}/bohb_pipe_{self.seed}.csv', 'a')
            f.write(prep_pipe_str + "\n")
            f = open(f'{self.output_dir}/bohb_score_{self.seed}.csv', 'a')
            f.write(str(score) + "\n")
            f = open(f'{self.output_dir}/bohb_eval_time_{self.seed}.csv', 'a')
...
def run_bohb(dataset, classifier, max_time_limit, max_len, seed, eta_value, budget_value):
    output_dir = make_output_dir_hb_vary_eta_and_budget(dataset, classifier, max_time_limit, algorithm='BOHB')

    np.random.seed(seed)

    min_budget = budget_value
    max_budget = 1
    port_num = 0
...
if __name__ == "__main__":
    ...
    ap.add_argument("-eta", "--eta", required=True,help="value of eta")
    ap.add_argument("-budget", "--budget", required=True,help="value of min budget")
    ...
    eta_value = int(args['eta'])
    budget_value = int(args['budget'])

    set_env(nthread=nthread)

    max_len = MAX_PIPE_LEN

    run_bohb(dataset, classifier, max_time_limit, max_len, seed, eta_value, budget_value)