...
class MissingValues(HelperFunction):
    ...
    def _calculate_sparse(self, X, y, logger):
    ...
    def _calculate(self, X, y, logger):
        if len(y.shape) == 2:
            ...
            for i in range(y.shape[1]):
                occurences.append(self._calculate(X, y[:, i], logger))
            return occurences
        else:
...
def calculate_all_metafeatures_encoded_labels(X, y, dataset_name, logger,
    ...
    return calculate_all_metafeatures(X, y, dataset_name,
                                      calculate=calculate,
                                      dont_calculate=dont_calculate, logger=logger)


def calculate_all_metafeatures_with_labels(X, y, dataset_name, logger,
                                           calculate=None, dont_calculate=None):
...
def calculate_all_metafeatures(X, y, dataset_name, logger,
    ...
    while len(to_visit) > 0:
        ...
        value = metafeatures[name](X_, y_, logger)
