...
def run_exp(dataset, classifier, max_time_limit, max_len, seed, K_num = 50):
    output_dir = make_output_dir(dataset, classifier, max_time_limit, algorithm='PNAS_MLP_ENSEMBLE')
    ...
    # Predictor
    predictor1 = MLPRegressor(random_state=seed)
    predictor2 = MLPRegressor(random_state=seed)
    predictor3 = MLPRegressor(random_state=seed)
    predictor4 = MLPRegressor(random_state=seed)
    predictor5 = MLPRegressor(random_state=seed)
    ...
    for i in range(len(operator_names)):
        ...
        f = open(f'{output_dir}/pnas_mlp_ensemble_pick_time_{seed}.csv', 'a')
        f.write(str(pick_time) + "\n")
        f = open(f'{output_dir}/pnas_mlp_ensemble_wallock_{seed}.csv', 'a')
        f.write(str(global_mid - global_start) + "\n")
        f = open(f'{output_dir}/pnas_mlp_ensemble_pipe_{seed}.csv', 'a')
        f.write(prep_pipe_str + "\n")
        f = open(f'{output_dir}/pnas_mlp_ensemble_score_{seed}.csv', 'a')
        f.write(str(score) + "\n")
        f = open(f'{output_dir}/pnas_mlp_ensemble_eval_time_{seed}.csv', 'a')
    ...
    kf = KFold(n_splits=5)
    predict_step = -1
    for train_index, test_index in kf.split(embedded_history):
        predict_step += 1
        print("TRAIN:", train_index, "TEST:", test_index)
        temp_embedded_his = []
        temp_validate_acc = []
        for item in train_index:
            temp_embedded_his.append(embedded_history[item])
            temp_validate_acc.append(validate_acc[item])
        predictors[predict_step].fit(temp_embedded_his, temp_validate_acc)
    ...
    for length in range(max_len - 1):
        ...
        for predictor in predictors:
            if (len(generate_acc) == 0):
                generate_acc = predictor.predict(generate_encode)
            else:
                generate_acc = np.sum([generate_acc, predictor.predict(generate_encode)], axis=0)
        generate_acc = generate_acc / len(predictors)
...
def run_pnas_mlp_ensemble(dataset, classifier, max_time_limit, max_len, seed):
...
if __name__ == "__main__":
    ...
    run_pnas_mlp_ensemble(dataset, classifier, max_time_limit, max_len, seed)
