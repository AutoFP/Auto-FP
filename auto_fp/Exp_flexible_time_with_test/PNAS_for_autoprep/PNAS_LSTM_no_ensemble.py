...
def run_exp(dataset, classifier, max_time_limit, max_len, seed, K_num = 50):
    output_dir = make_output_dir(dataset, classifier, max_time_limit, algorithm='PNAS_LSTM_NO_ENSEMBLE')
    ...
    # Predictor
    predictor = LSTM_predictor(seed=seed)
    ...
    for i in range(len(operator_names)):
        ...
        if global_mid - global_start >= max_time_limit:
            ...
            time_limit_reached = False
        f = open(f'{output_dir}/pnas_lstm_no_ensemble_pick_time_{seed}.csv', 'a')
        f.write(str(pick_time) + "\n")
        f = open(f'{output_dir}/pnas_lstm_no_ensemble_wallock_{seed}.csv', 'a')
        f.write(str(global_mid - global_start) + "\n")
        f = open(f'{output_dir}/pnas_lstm_no_ensemble_pipe_{seed}.csv', 'a')
        f.write(prep_pipe_str + "\n")
        f = open(f'{output_dir}/pnas_lstm_no_ensemble_score_{seed}.csv', 'a')
        f.write(str(score) + "\n")
        f = open(f'{output_dir}/pnas_lstm_no_ensemble_eval_time_{seed}.csv', 'a')
    ...
    no_encode_history_tensor = Variable(torch.from_numpy(np.array(no_encode_history)).type(torch.LongTensor))
    validate_acc_tensor = Variable(torch.from_numpy(np.array(validate_acc)).type(torch.FloatTensor))
    criterion = torch.nn.MSELoss()
    optimizer = torch.optim.Adam(predictor.parameters(), lr=0.01)

    for epoch in range(100):
        for train_idx in range(len(no_encode_history_tensor)):
            predictor.zero_grad()
            predictor.hidden = predictor.init_hidden()
            y_pred = predictor(no_encode_history_tensor[train_idx])
            loss = criterion(y_pred, validate_acc_tensor[train_idx])
            loss.backward()
            optimizer.step()
        print(epoch)
    ...
    for length in range(max_len - 1):
        ...
        for i in range(len(temp_pipes[-1])):
            ...
            for j in range(len(operator_names)):
                ...
                generate_acc.append(predictor(Variable(torch.from_numpy(np.array(pre_pipe + [j])).type(torch.LongTensor)))[0][0][0])
        ...
        for epoch in range(100):
            for train_idx in range(len(no_encode_history)):
                predictor.zero_grad()
                predictor.hidden = predictor.init_hidden()
                y_pred = predictor(Variable(torch.from_numpy(np.array(no_encode_history[train_idx])).type(torch.LongTensor)))
                loss = criterion(y_pred, validate_acc_tensor[train_idx])
                loss.backward()
                optimizer.step()
            print(epoch)
...
def run_pnas_lstm_no_ensemble(dataset, classifier, max_time_limit, max_len, seed):
...
if __name__ == "__main__":
    ...
    run_pnas_lstm_no_ensemble(dataset, classifier, max_time_limit, max_len, seed)
