...
class Intensifier(AbstractRacer):
    ...
    def get_next_challenger(self,
        ...
        if self.current_challenger and self.continue_challenger:
            #end = time.time()
            #f = open('smac_pick_time_1.csv', 'a')
            #f.write(str(end - start) + "\n")
            #print("Pick up time: " + str(end - start))
        ...
        if not self.current_challenger or \
            ...
            if self.update_configs_to_run:
                configs_to_run = self._generate_challengers(challengers=challengers, chooser=chooser, output_dir=output_dir, output_seed=output_seed)
    ...
    def _generate_challengers(self,
        ...
        if challengers:
            ...
            chall_gen = chooser.choose_next(dataset=dataset, classifier=classifier, max_time_limit=max_time_limit)
        else:
